...
class Scanner:
    def __init__(self,filename):
        ...
        self.__faForIdentifier = FiniteAutomata("FAIdentifier.in")
        self.__faForIntegerConstant = FiniteAutomata("FAIntegerConstant.in")
    ...
    def isOperator(self,line,index): #look-ahead
        if ( index+1<len(line) ) and ( line[index] in operators ) and ( line[index+1] in operators ) and ( line[index]+line[index+1] in operators ):
            if (index+2==len(line)) or (line[index+2] in separators) or (line[index+2]=='"') or (line[index+2] in digitsAndMinus) or containsLetters(line[index+2]):
                return True
        elif line[index] in operators:
            if (index+1==len(line)) or (line[index+1] in separators) or (line[index+1]=='"') or (line[index+1] in digitsAndMinus) or containsLetters(line[index+1]):
                return True
    ...
    def isNumber(self, line, index):
        ...
        while len(line) > index and (line[index] not in separators) and (line[index] not in operators):
            string += line[index]
            index += 1
        if self.__faForIntegerConstant.isAccepted(string):
            return True
        return False
    ...
    def isIdentifier(self,line,index):
        ...
        while len(line)>index and (line[index] not in separators) and (line[index] not in operators):
            string += line[index]
            index+=1
        if self.__faForIdentifier.isAccepted(string):
            return True
        return False
...
scanner = Scanner('p2.in')
scanner.getTokens()
print(scanner.isNumber("-1112",0))
